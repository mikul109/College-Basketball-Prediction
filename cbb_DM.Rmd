...
cbb1$SEED[cbb1$SEED==100]<-0
# All teams that made the playoffs are given a value of 1
cbb1$SEED[cbb1$SEED>0]<-1
View(cbb1)
# subset with needed variables
cbb1_mod <- subset(cbb1, select=c(SEED, W, ADJOE, ADJDE, EFG_O, EFG_D, DRB))

# randomly split data 70/30
set.seed(123)
ind <- sample(2, nrow(cbb1_mod), replace=TRUE, prob=(c(0.7,0.3)))
train_data_cbb1 <- cbb1_mod[ind==1,]
test_data_cbb1 <- cbb1_mod[ind==2,]


cbblmBinary<-glm(SEED~W, data =train_data_cbb1, family="binomial")
summary(cbblmBinary)
# Make predictions
pred_classes_lmBin <- cbblmBinary %>% predict(test_data_cbb1)
# Model accuracy
pred_
```
...
```{r}
plot(cbb_svm, train_data, EFG_O~TOR)
plot(W~ADJOE,data=cbb)
```
